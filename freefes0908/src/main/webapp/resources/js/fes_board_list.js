		    ...
		    success: function (response) {
		        ...
		        if (typeof board_list != 'string') {
		            board_list.forEach(function (board) {
		            	const formatted_start_date = formatDate(board.start_date)	// 시작일 ''월 ''일 형식 포맷
	            		const formatted_end_date = formatDate(board.end_date)	// 끝일 ''월 ''일 형식 포맷
	            		
	            		const filePath = board.file_path	// 파일 경로
	            		const filePath_length = filePath.length		// 파일 경로 String 길이
	            		const lastDotIndex = filePath.indexOf('poster.')	// 파일 경로 중 poster. 인덱스 번호
	            		const fileExtension = filePath.subString(lastDotIndex + 7, filePath_length)		// 파일 경로 poster. 뒤 확장자
	            		
		                let festival_list = `
		                <div class="festival_list">
			                <div class="festival_poster">
							<a href="${cpath }/fes_board/mainboard/${board.idx }">
								<img src="${cpath }/upload/${board.title }/poster.${fileExtension}">
							</a>
							</div>
							<div>
	                             <a href="${cpath}/fes_board/mainboard/${board.idx}">
...
$(document).ready(function() {
	$(".fes_board_paging").on("click", "a", function(event) {
		event.preventDefault()
...
function update_paging(paging_data, query_string) {
	...
	if (paging_data != null) {
		...
		for (let page_number = paging_data.page_begin; page_number <= paging_data.page_end; page_number++) {
			fes_board_paging.append(`<a class="page_link" href="${cpath}/fes_board/fes_board_list?` + query_string + `${page_number}">[${page_number}]</a>`);
		}
		
		if (paging_data.next && paging_data.page_end < paging_data.total_count) {
			fes_board_paging.append(`<a class="page_link" href="${cpath}/fes_board/fes_board_list?` + query_string + `${paging_data.page_end + 1}">다음▶</a>`);
		}
	}
	else {
		fes_board_paging.empty();
...
//$(document).ready(function() {
//    // 이전 페이지에서 세션 데이터 검색
//    const searchResult = sessionStorage.getItem("search_result");
//
//    if (searchResult) {
...
//        resultDiv.empty();
//
//        if (Array.isArray(searchData.fes_boardList)) {
//            searchData.fes_boardList.forEach(function (board) {
//                const festivalList = `
//                    <div class="festival_list">
//                        <div class="festival_poster"></div>
//                        <div>
//                            <a href="${cpath}/fes_board/mainboard/${board.idx}">
...
//                        </div>
//                    </div>`;
//                $(festivalList).appendTo(resultDiv);
//            });
//        } else {
//            const message = searchData.fes_boardList.toString();
//            resultDiv.html(message);
//        }
//
//        // 페이지 번호 업데이트 코드 (기존 코드를 그대로 사용)
//        let query_string = null;
//        const form_data = $("#fes_search_form").serialize();
//        if (form_data.includes('&request_page=')) {
//            query_string = form_data.substring(0, form_data.indexOf('&request_page='));
//            query_string += '&request_page=';
//        } else {
...
//        }
//
//        update_paging(searchData.fes_paging_dto, query_string);
//    }
//});

/*	동영 추가	*/

const grades = document.querySelectorAll(".board_grade")	// 평점 선택
