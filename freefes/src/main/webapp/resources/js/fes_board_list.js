	    ...
	    success: function (response) {
	        ...
	        if (typeof board_list != 'string') {
	            board_list.forEach(function (board) {
            		...
            		const poster = filePath.subString(0, filePath.indexOf('&')) // 파일 경로 poster. 뒤 확장자
	            	
	                let festival_list = `
	                 <div class="festival_list">
			                <div class="festival_poster">
							<a href="${cpath }/fes_board/mainboard/${board.idx }">
								<img src="${cpath }/upload/${board.title }/${poster}">
							</a>
							</div>
							<div>
	                             <a href="${cpath}/fes_board/mainboard/${board.idx}">
...
$(document).ready(function() {
	$(".fes_board_paging").on("click", ".update_page", function(event) {
		event.preventDefault()
...
function update_paging(paging_data, query_string) {
	...
	if (paging_data != null) {
		...
		if (paging_data.prev && paging_data.page_begin > 1) {
			fes_board_paging.append(`<a class="update_page" href="${cpath}/fes_board/` + fes_page_href + `?` + query_string + `${paging_data.page_begin - 1}">◀이전</a>`);
		}
		
		for (let page_number = paging_data.page_begin; page_number <= paging_data.page_end; page_number++) {
			fes_board_paging.append(`<a class="update_page" href="${cpath}/fes_board/` + fes_page_href + `?` + query_string + `${page_number}">[${page_number}]</a>`);
		}
		
		if (paging_data.next && paging_data.page_end < paging_data.total_count) {
			fes_board_paging.append(`<a class="update_page" href="${cpath}/fes_board/` + fes_page_href + `?` + query_string + `${paging_data.page_end + 1}">다음▶</a>`);
		}
	}
	else {
		fes_board_paging.empty();
