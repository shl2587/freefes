...
<div class="management_main_flex">
	<div class="management_title">
		<h3 style="color: #F0B86E; font-size:23px;">Management /
			<span style="color: #FF6969; font-size:27px;">ReceiveMessage-List</span>
		</h3>
	</div>
</div>
<hr>
	...
	<table class="management_message_table">
	  <thead>
	    <tr>
			...
			<th>Category</th>
			<th>Date</th>
			<th class="listMessage_table_th"><input type="checkbox" class="message-checkbox_All" data-idx="${dto.idx}">Delete-All</th>
	    </tr>
	  </thead>
	  <tbody>
...
<div class="page_wrap">
   <div class="page_nation">
      <c:if test="${paging_dto.prev && paging_dto.page_begin > 1}">
	      <a class="arrow prev" href="${cpath }/admin_board/management_receiveMessage_list?request_page=${paging_dto.page_begin - paging_dto.per_page}"></a>
	  </c:if>
	  <c:forEach var="page_number" begin="${paging_dto.page_begin }" end="${paging_dto.page_end }">
      	  <a href="${cpath }/admin_board/management_receiveMessage_list?request_page=${page_number}" class="active">${page_number }</a>
      </c:forEach>
      <c:if test="${paging_dto.next && paging_dto.page_end < paging_dto.total_count}">
      	  <a class="arrow next" href="${cpath }/admin_board/management_receiveMessage_list?request_page=${paging_dto.page_end + 1 }"></a>
      </c:if>
   </div>
</div>
